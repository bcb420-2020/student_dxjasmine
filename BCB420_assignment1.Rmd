...
```{r}
samples <- data.frame(
  lapply(colnames(exp_data)[2:9], 
         FUN=function(x){
           unlist(strsplit(unlist(strsplit(x, split = "_"))[2],"-"))[c(1,2)]})
  )
colnames(samples) <- colnames(exp_data)[2:9]
rownames(samples) <- c("cell line","cell type")
...
```
The MDS plot show that samples have smaller distance within same cell line within both control and G9a group. 
...
exp_data_df <- data.frame(matrix(unlist(exp_data_filtered$gname), ncol =1, byrow = T))
for (col in colnames(exp_data_filtered)[-1]) {
  exp_data_df <- cbind(exp_data_df,data.frame(matrix(unlist(exp_data_filtered[col]), ncol =1, byrow = T)))
}

names(exp_data_df) <- colnames(exp_data_filtered)

#########################
hgnc_check <- read.csv("data/hgnc-symbol-check.csv",header = FALSE)
hgnc_check <- as.data.frame(hgnc_check)
hgnc_check <- hgnc_check[-1:-3,]
sum(hgnc_check$V2== "Approved symbol")
matched_symbol <- hgnc_check$V1[hgnc_check$V2== "Approved symbol"]
matched_symbol <- hgnc_check$V1[hgnc_check$V2== "Approved symbol"]
exp_data_filtered_df <- exp_data_df[matched_symbol,]
sum(is.na(exp_data_df$gname))
```
...
exp_data_df <- data.frame(matrix(unlist(exp_data_filtered$gname), ncol =1, byrow = T))
for (col in 1:8) {
  exp_data_df <- cbind(exp_data_df, data.frame(matrix(unlist(normalized_counts[,col]))))
}
